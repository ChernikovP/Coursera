...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should work for multi-coins") {
    ...
    def moneyThresholdCheck(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, moneyThreshold(money)) == expected,
        s"parCountChange($money, $coins, moneyThreshold($money)) should be $expected")

    def coinsThresholdCheck(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, totalCoinsThreshold(coins.length)) == expected,
        s"parCountChange($money, $coins, totalCoinsThreshold($coins.length)) should be $expected")

    def combinedThresholdCheck(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, combinedThreshold(money, coins)) == expected,
        s"parCountChange($money, $coins, combinedThresholdCheck($money, $coins)) should be $expected")

    check(50, List(1, 2, 5, 10), 341)
    check(250, List(1, 2, 5, 10, 20, 50), 177863)

    moneyThresholdCheck(50, List(1, 2, 5, 10), 341)
    moneyThresholdCheck(250, List(1, 2, 5, 10, 20, 50), 177863)

    coinsThresholdCheck(50, List(1, 2, 5, 10), 341)
    coinsThresholdCheck(250, List(1, 2, 5, 10, 20, 50), 177863)

    combinedThresholdCheck(50, List(1, 2, 5, 10), 341)
    combinedThresholdCheck(250, List(1, 2, 5, 10, 20, 50), 177863)
  }
}
