...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    if (meansMap.size == means.size) meansMap
    else {
      val emptyMeansMap = means.filter(!meansMap.contains(_)).map(m => m -> GenSeq[Point]()).toMap

      meansMap ++ emptyMeansMap
    }
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] =
    oldMeans.map(m => findAverage(m, classified.getOrElse(m, GenSeq[Point]())))

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean =
    oldMeans.zip(newMeans).forall { case(o, n) => o.squareDistance(n) <= eta }

  @tailrec
  // your implementation needs to be tail recursive
  final def kMeans(points: GenSeq[Point], oldMeans: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val classified = classify(points, oldMeans)
    val newMeans = update(classified, oldMeans)

    if (converged(eta)(oldMeans, newMeans)) newMeans
